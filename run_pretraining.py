...
def parse_arguments():
    ...
    random_number = generate_random_number(1, 1000000000000)
    random_number=str(random_number)
    args.job_name = f"{args.job_name}-{random_number}"
    logger.info(f"Running Config File: {args.job_name}")
    logger.info(f"Args = {args}")
    os.makedirs(args.output_dir, exist_ok=True)
    args.saved_model_path = os.path.join(args.output_dir, args.job_name, random_number)
    return args
...
def prepare_model_and_optimizer(args):
    ...
    # Write model parameters before DeepSpeed initialization to file
    # total_params_before = 0
    # with open("parameters_before_adam_fp32.txt", "w") as f:
    #     for name, param in model.network.named_parameters():
    #         f.write(f"Name: {name}, Size: {param.size()}, dtype: {param.dtype}\n")
    #         total_params_before += param.numel()
    #     f.write(f"Total parameters: {total_params_before}\n")
    ...
    # # Write model parameters after DeepSpeed initialization to file
    # total_params_after = 0
    # with open("parameters_after_adam_fp32.txt", "w") as f:
    #     for name, param in model.network.named_parameters():
    #         f.write(f"Name: {name}, Size: {param.size()}, dtype: {param.dtype}\n")
    #         total_params_after += param.numel()
    #     f.write(f"Total parameters: {total_params_after}\n")
...
def start_training(args, model, optimizer, lr_scheduler, start_epoch):
    ...
    for index in range(start_epoch, args.num_epochs):
        ...
        # save a checkpoint
        if (index > 0 and args.num_epochs_between_checkpoints > 0
