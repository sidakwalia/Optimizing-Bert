...
"""Import required libraries and set global constants.

Define get_current_memory_gb() function to get the memory usage of the process.

Define parse_args() function to parse command-line arguments. This function sets up an argument parser with various options for input and output datasets, sharding parameters, cache directory, and other options.

Define distribution() function that distributes the data to a certain number of partitions. This function is responsible for:

Creating the output directories for shards.
Processing Huggingface datasets:
Loading the dataset using the load_dataset() function.
Saving articles into .txt files.
Processing custom datasets:
Reading .txt files from the specified directory.
Saving articles into .txt files.
Writing the file list to a temporary directory.
In the if __name__ == '__main__': block:

Parse command-line arguments.
Create a temporary directory and communication file for multi-machine processing.
Check if the current machine should perform the dataset distribution.
If the dataset distribution is completed, move to the next step.
Otherwise, wait for the dataset distribution to be completed.
Update the progress bar (if used).
Read the file list from the temporary directory.
Calculate the train and test shard ranges for the current machine.
Initialize the main sharding process by creating an instance of TextSharding.Sharding.
Distribute articles over shards using the distribute_articles_over_shards() method.
Write shards to disk using the write_shards_to_disk() method.
Close the progress bar (if used).
The purpose of this script is to partition large text datasets into smaller, more manageable pieces (shards) that can be easily processed in parallel across multiple machines. This can be particularly useful when working with large-scale machine learning models, like those found in natural language processing tasks."""
