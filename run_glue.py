...
class FinetuneTrainingArguments(TrainingArguments):
    ...
    project_name: Optional[str] = field(default="final_version_fp16_adamw_4k", metadata={"help": "Project name (W&B)"})
    ...
    finetune_checkpoint_at_end: Optional[bool] = field(
        default=False,
        metadata={"help": "Save a finetuning checkpoint (for the second finetuning later) when training (this first finetuning) is done."},
    )


def create_finetune_job(args, pretrain_run_args, tokenizer, model):
    try:
        ...
        # layer and throw an error to complain when the number of labels differ
        model.bert.save_pretrained(save_model_path)
        tokenizer.save_pretrained(save_model_path)

        # We passes pretrain run args to downstream finetune tasks
        # They may need its deepspeed args for corresponding adjustment
        args_file_path = os.path.join(save_model_path, "args.json")
        args_dict = to_sanitized_dict(pretrain_run_args)

        from pretraining.base import BasePretrainModel
        BasePretrainModel.to_json_file(args_dict, args_file_path)
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        print("the model args ",training_args)
    ...
    if training_args.do_train:
        ...
        # For some tasks, i.e. RTE, MRPC, STSB, we need first finetune on
        # MNLI before finetuning on those tasks
        if is_main_process(training_args.local_rank) and training_args.finetune_checkpoint_at_end:
            create_finetune_job(
                args=training_args,
                pretrain_run_args=pretrain_run_args,
                tokenizer=tokenizer,
                model=model)
    ...
    if training_args.do_predict:
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            test_results_file_name = f"predict_results_{task}.txt"
